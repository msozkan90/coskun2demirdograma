    ...
    <div class="col-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <form class="forms-sample" enctype="multipart/form-data" action="{{route('project_update',['id' => $project_data[0]['id']])}}" method="POST">
                    @csrf
                    @if ($errors->any())
                    ...
                    <div class="form-group">
                        ...
                        <input type="month" value="{{$project_data[0]['started-date']}}"   class="form-control" name="started-date" id="started-date" placeholder="Started Date">
                    </div>
                    <div class="form-group">
                        <label for="finished-date">Bitiş Tarihi</label>
                        <input type="month" value="{{$project_data[0]['finished-date']}}"  class="form-control" name="finished-date" id="finished-date" placeholder="Finish Date">
                    </div>

                    <div class="form-group">
                        <label for="title">Proje Adı</label>
                        <input type="text" class="form-control" value="{{$project_data[0]['title']}}"   name="title" id="title" placeholder="Proje Adı">
                    </div>
                    <div class="form-group">
                        <label for="location">Projenin Yeri</label>
                        <input type="text" class="form-control" value="{{$project_data[0]['location']}}" name="location" id="location" placeholder="Projenin Yeri">
                    </div>
                    <div class="form-group">
                        <label for="image">Proje Kapak Fotoğrafı</label>
                        <input type="file" class="form-control" value="{{$project_data[0]['image']}}"  name="image" id="image" placeholder="Proje Kapak Fotoğrafı">
                    </div>
